...
/* ***************************** EXERCICE 2 ***************************** */
 ...
 void PW(Graphe *G, int col[]){
    ...
    tri_decroi(d,dpoint,G->n);

    // Ici on a donc d avec les degrès dans l'ordre décroissant, et dpoint avec les points associés
    ...
    for (i=0;i<G->n;i++){
        ...
        for (j=0;j<G->n;j++){
            if (i!=j){
                if (est_adjacent_direct(G,dpoint[i],dpoint[j]) && (col[i]==col[j])){
                        printf("%d \n",est_adjacent_direct(G,1,4));
                    //printf("d[i] : %d et d[j] : %d \n",dpoint[i],dpoint[j]);
                    col[i]=col[i]+1;}
            }
            else{
    ...
    //On obtient donc ici, d avec les degrès dans l'ordre décroissant, et dpoint avec les points associés et col avec les couleurs.
...
void tri_decroi(int td[],int tpts[],int taille){
    int tmp,maxi,deg,i,j;
